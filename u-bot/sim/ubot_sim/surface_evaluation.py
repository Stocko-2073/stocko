...
def main():
    ...
    files = [Path(__file__), *[Path(__file__).with_name(n) for n in
             ("contact_model.py", "env.py", "waypoints.py", "elevation.py", "grass.py", "canopy.py")],
             Path(__file__).parent / "assets" / "robot.xml"]
    from ubot_sim.canopy import GrassCanopy
    report = {"grass_canopy": asdict(GrassCanopy()), "presets": {name: asdict(value) for name, value in SURFACE_PRESETS.items()},
