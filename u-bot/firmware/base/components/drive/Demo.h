...
// Choreographed runs: pick a script, start it, and film -- or use the short as
// the two-wheel acceptance test it also is.
//
// Non-blocking on purpose. Calibration blocks and telemetry stops dead while it
// runs, which is fine for a measurement and useless for a video -- this is
// driven from the control tick instead, so the stream keeps flowing.
//
...
// that segment; edit the table, not the runner. A caption is logged as it
// starts, so the log doubles as a shot list.
//
...
// calibration can supply, and it arrives here as Axis::sign -- the same sign
// the drive layer uses for joystick commands.
class Demo {
  public:
    ...
    struct Beat {
        const char *say;    // caption, logged when the beat starts
    ...
    static const Script SHORT;  // both wheels, ~9 s, the two-wheel check

    // Needs the drivers already enabled -- energising a motor stays an explicit
    // act. Logs its own reason and returns false rather than starting a take
    // that cannot finish.
    ...
    bool start(const Script &sc, const Axis &a, const Axis &b) {
        ...
        if (sc.pair) {
            if (!b.servo) {
                ESP_LOGE(TAG, "this script drives both wheels and only one was given");
                return false;
            }
            _ax[_n++] = b;
        }
        ...
        for (uint8_t i = 0; i < _n; i++) {
            if (!_ax[i].servo->driverEnabled()) {
                ESP_LOGE(TAG, "enable the drivers first -- a demo will not energise a motor for you");
                return false;
            }
            if (!_ax[i].servo->encoderOk()) {
                ESP_LOGE(TAG, "wheel %s's encoder is not responding -- nothing to close the loop on",
                         _ax[i].name);
                return false;
            }
        }
        ...
        // has to hand things back the way it found them.
        for (uint8_t i = 0; i < _n; i++) {
        ...
        _running = true;
        _startMs = millis();
        ESP_LOGI(TAG, "%s: %u beats, ~%.1f s planned -- roll camera",
    ...
    void stop() {
        ...
        restore();
        _caption = "stopped";
        ESP_LOGI(TAG, "%s: stopped", _sc->name);
    }
    ...
    void update() {
        ...
        // a pair script either wheel ends it.
        for (uint8_t i = 0; i < _n; i++) {
            if (_ax[i].servo->fault() == StepperServo::FAULT_NONE) continue;
            ESP_LOGW(TAG, "%s: abandoned at beat %u -- wheel %s %s",
        ...
        if (_arriving) {
            ...
            if (!allAtTarget()) {
                for (uint8_t i = 0; i < _n; i++) {
                    ...
                    ESP_LOGW(TAG, "%s: beat %u timed out, wheel %s %ld counts short, carrying on",
                             _sc->name, (unsigned)(_i + 1), _ax[i].name,
  ...
  private:
    static constexpr const char *TAG = "demo";
    ...
    void applyBeat() {
        ...
        if (b.say) {
            _caption = b.say;
            ESP_LOGI(TAG, "%s %u/%u: %s", _sc->name, (unsigned)(_i + 1),
    ...
    void finish() {
        ...
        restore();
        _caption = "done";
        ESP_LOGI(TAG, "%s: done in %.1f s (planned %.1f)", _sc->name,
                 (millis() - _startMs) * 0.001f, _planned);
        for (uint8_t i = 0; i < _n; i++) {
            int32_t err = _ax[i].servo->positionCounts();
            ESP_LOGI(TAG, "  wheel %s back at %+ld counts from where it started"
                          " (%.2f mm at the rim, %.2f deg)",
                     _ax[i].name, (long)err, err * MM_PER_COUNT,
                     err * 360.0f / StepperServo::COUNTS_PER_REV);
        }
        ESP_LOGI(TAG, "loops still closed and holding -- 'stop' to release");
    }
    ...
    // the ideal profile. Logged before the take so a script can be trimmed to
    // length without filming it first.
    ...
    // for walking in the last few counts. Measured on 2026-08-31 at 0.04 s: a
    // beat only waits for the tolerance band, not for a standstill.
...
// The short: both wheels, one pattern, ~9 s planned. Three phases:
//
...
// round-trip check as well as an outro. If "in phase" counter-rotates the
// wheels instead of rolling them the same way, `sign_b` is wrong -- flip that
