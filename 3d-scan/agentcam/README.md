...
mat, a sheet of markers printed from `3d-scan/workmat_aruco_4x4_10mm.pdf` at 100%. The iPhone app locks
onto the mat, guides you to each pose with two wireframe cubes and a sight line, takes the photo
...
A `freeform` request has no pose: it's taken as soon as the whole mat is in view with the phone steady,
...
| `Packages/AgentCamCore/` | Protocol, frames, mat-pose fusion, guidance, link, outbox. Plain Swift, tested on the Mac. |
| `Packages/AgentCamVision/` | OpenCV marker detection and the mat solver (C++ bridge), the same algorithm as the server. |
| `protocol/` | The shared contract: mat layouts, JSON examples both sides decode, and the real-photo fixture. |
...
`cancel_requests`, `phone_status`, `get_mat`, `set_print_scale`. `get_mat` explains the frames:
mat frame in mm, origin at the mat center, +y toward the marker-0 edge, +z up; OpenCV camera axes on
...
each queued request, renders the mat from that pose, and uploads it through the real protocol.
...
- **The mat won't lock.** It needs to be flat and level (within 20 degrees of level), with markers
...
Done: mat lock, guidance, automatic capture on the main lens (12 MP), torch, focus, exposure and
white-balance locks, ARKit LiDAR depth, delivery with still-image PnP checks, placement prompts,
"can't reach" skips with a turn-the-mat tip, freeform requests.
