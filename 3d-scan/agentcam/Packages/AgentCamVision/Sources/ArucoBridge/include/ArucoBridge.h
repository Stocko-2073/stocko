...
/// AgentCam: (0, 0) is the centre of the top-left pixel.
typedef struct {
    int32_t id;
    float x[4];
    float y[4];
...
/// The page's pose from all detected markers, the same algorithm as the
/// server's enrich.solve_board: RANSAC (AP3P) over every corner; a marker is
