...
/// The page's pose in one image, from its detected markers.
public struct BoardFit: Sendable {
    public struct Solution: Sendable {
        public var cameraFromPage: Pose     // OpenCV camera axes, mm
    ...
    /// away is ambiguous; BoardPoseFusion picks with gravity and history.
...
public enum BoardSolver {
    /// RANSAC inlier threshold: 35% of the median marker side (3.5 mm on the
    /// paper): generous for a curled page, far below a misplaced marker.
    ...
    /// Same algorithm as the server's enrich.solve_board (see ac_solve_board).
    public static func solve(_ markers: [DetectedMarker], layout: BoardLayout, k: simd_double3x3) -> BoardFit {
        var fit = BoardFit(markers: markers, inliers: Array(repeating: false, count: markers.count), solutions: [])
        ...
        let n = ac_solve_board(ids, object, image, Int32(known.count), kRowMajor,
        ...
        fit.solutions = out.prefix(Int(n)).map { p in
            ...
            return BoardFit.Solution(cameraFromPage: Pose(rotation: rotation, translation: SIMD3(t[0], t[1], t[2])),
                                     rmsPx: p.rmsPx)
        }
        return fit
    }
