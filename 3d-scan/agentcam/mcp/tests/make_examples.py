...
the two sides of the protocol in sync. Rerun after changing models.py or the
...
from agentcam.models import PROTOCOL_VERSION, MatInfo, PhotoRequest, PhotoRequestSpec, PoseSpec
...
def pose_examples() -> list[dict]:
    ...
    for name, spec in POSE_SPECS.items():
        ...
        out.append({"name": name, "spec": spec, "camera_to_mat": np.round(t, 9).tolist(),
...
def requests_snapshot() -> dict:
    ...
    specs = [
        ...
        PhotoRequestSpec.model_validate({
            ...
            "placement": {"label": "flipped", "instruction": "Turn the battery upside down, centered on the mat."}}),
        PhotoRequestSpec.model_validate({"kind": "freeform", "options": {"torch": 0.5, "depth": "arkit"},
                                         "note": "first look at what's on the mat"}),
    ]
    items = []
    for i, spec in enumerate(specs):
        target, warnings = resolve(spec, mat)
...
STATUS = {
    ...
    "tracking": {"arkit": "normal", "mat": {"locked": True, "age_s": 0.3, "markers": 23, "rms_px": 0.6,
                                              "tilt_deg": 1.1, "jitter_mm": 0.3, "generation": 1}},
    "camera_to_mat": [[1, 0, 0, 0], [0, -0.5736, 0.8192, -245.7], [0, -0.8192, -0.5736, 192.1], [0, 0, 0, 1]],
...
CAPTURE_META = {
    ...
    "pose": {"camera_to_mat": [[1, 0, 0, 0], [0, -0.5736, 0.8192, -245.7], [0, -0.8192, -0.5736, 192.1], [0, 0, 0, 1]],
             "source": "arkit_live",
    ...
    "exposure": {"duration_s": 0.008, "iso": 200, "bias_ev": 0, "wb_gains": [2.0, 1.0, 1.7], "flash_fired": False,
                 "torch": 0},
    "tracking": {"arkit": "normal", "mat_age_s": 0.2, "mat_generation": 1, "tilt_deg": 1.1, "speed_mm_s": 4,
                 "ang_speed_deg_s": 0.6},
...
def main() -> None:
    ...
    files = {
        ...
        "welcome.json": {"t": "welcome", "v": PROTOCOL_VERSION, "server_id": "a1b2c3d4e5f6",
                         "mat": MatInfo().model_dump(mode="json")},
