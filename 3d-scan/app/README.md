...
```
Claude Code ──stdio──> agentcam-mcp (app/mcp) <──Wi-Fi: Bonjour, WebSocket, HTTP──> AgentCam (iPhone)
                            │
...
```sh
cd 3d-scan/app/mcp && uv sync --group dev && uv run pytest -q          # server: 33 tests
