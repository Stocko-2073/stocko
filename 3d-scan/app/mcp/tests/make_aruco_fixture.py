"""Regenerate app/protocol/fixtures/aruco_border.json, the reference solution for
the real photo of the printed page (aruco_border.jpeg, 12 MP, portrait hold).
...
here with the principal point fixed at the centre and no distortion; the pose
...
from agentcam.enrich import solve_board
...
def main() -> None:
    ...
    fit = solve_board(board, ids, corners, guess)
    obj = board.object_points(ids[fit.inliers]).astype(np.float32)
    ...
    fit = solve_board(board, ids, corners, k)
    best = fit.choose()
    out = {
        "image": "aruco_border.jpeg", "size": [w, h], "dictionary": board.dictionary,
        ...
        "camera_to_page": np.round(best.camera_to_page, 9).tolist(),
        "rms_px": round(best.rms_px, 6),
        "solutions": [{"camera_to_page": np.round(s.camera_to_page, 9).tolist(), "rms_px": round(s.rms_px, 6)}
    ...
    print(f"{len(ids)} detections, {int(fit.inliers.sum())} inliers, rejected {out['rejected_ids']}, "
          f"f {k[0, 0]:.1f} px, rms {best.rms_px:.2f} px, camera at {best.camera_to_page[:3, 3].round(1)} mm")
